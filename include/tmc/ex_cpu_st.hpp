...
class ex_cpu_st {
  ...
  TMC_DEF void notify_n(size_t Priority, bool FromExecThread);
  ...
  TMC_DEF bool try_run_some(std::stop_token& ThreadStopToken, size_t& PrevPriority);
  ...
  TMC_DEF void set_state(WorkerState NewState);
  TMC_DEF WorkerState get_state();
...
public:
  ...
  TMC_DEF ex_cpu_st& add_partition(tmc::topology::topology_filter Filter);
#endif
  ...
  /// The default is 1.
  TMC_DEF ex_cpu_st& set_priority_count(size_t PriorityCount);
#endif
  ...
  TMC_DEF ex_cpu_st& set_thread_init_hook(std::function<void(size_t)> Hook);
  ...
  TMC_DEF ex_cpu_st& set_thread_teardown_hook(std::function<void(size_t)> Hook);
  ...
  TMC_DEF ex_cpu_st& set_spins(size_t Spins);
...
template <> struct executor_traits<tmc::ex_cpu_st> {
  ...
  static TMC_DEF tmc::ex_any* type_erased(tmc::ex_cpu_st& ex);

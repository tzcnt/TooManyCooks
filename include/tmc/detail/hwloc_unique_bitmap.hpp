...
struct hwloc_unique_bitmap {
  ...
  TMC_DECL_HWLOC hwloc_unique_bitmap();
  TMC_DECL_HWLOC hwloc_unique_bitmap(hwloc_bitmap_s*);

  // Releases the bitmap on destruction. If null, nothing happens.
  TMC_DECL_HWLOC ~hwloc_unique_bitmap();

  // Explicitly releases the bitmap early.
  TMC_DECL_HWLOC void free();
  ...
  TMC_DECL_HWLOC hwloc_unique_bitmap clone();

  // Can be moved, transferring ownership of the bitmap
  TMC_DECL_HWLOC hwloc_unique_bitmap(hwloc_unique_bitmap&& Other);
  TMC_DECL_HWLOC hwloc_unique_bitmap& operator=(hwloc_unique_bitmap&& Other);

  TMC_DECL_HWLOC operator hwloc_bitmap_s*();

#ifdef TMC_DEBUG_THREAD_CREATION
  TMC_DECL_HWLOC void print();
#endif

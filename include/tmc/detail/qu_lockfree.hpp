  ...
  struct Block {
    ...
    template <InnerQueueContext context> inline bool is_empty() const {
      if constexpr (context == explicit_context) {
        size_t flagsAccumulator = BLOCK_EMPTY_MASK;
        for (size_t i = 0; i < BLOCK_EMPTY_ARRAY_SIZE; ++i) {
#ifdef MOODYCAMEL_HAS_TSAN
...
#endif
          flagsAccumulator &= emptyFlags[i].load(std::memory_order_relaxed);
        }
        if (flagsAccumulator != BLOCK_EMPTY_MASK) {
          return false;
        }
    ...
    enqueue_bulk(It itemFirst, size_t count) {
      ...
      if (blockBaseDiff > 0) [[unlikely]] {
        ...
        while (blockBaseDiff > 0) {
          ...
          newBlock->owner = this;
#endif
          // Is this really necessary? don't we reset_empty right afterward?

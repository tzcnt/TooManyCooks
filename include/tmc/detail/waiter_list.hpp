...
struct waiter_list_waiter {
  ...
  [[nodiscard]] TMC_DEF std::coroutine_handle<>
...
public:
  ...
  /// Thread-safe.
  TMC_DEF void add_waiter(waiter_list_node& w) noexcept;

  /// Wakes all waiters. Head becomes nullptr.
  /// Thread-safe.
  TMC_DEF void wake_all() noexcept;

  /// Returns head. Head becomes nullptr.
  /// Thread-safe.
  [[nodiscard]] TMC_DEF waiter_list_node* take_all() noexcept;

  /// Assumes at least 1 waiter is in the list.
  /// Takes 1 waiter.
  /// Not thread-safe.
  [[nodiscard]] TMC_DEF waiter_list_node* must_take_1() noexcept;
  ...
  TMC_DEF waiter_list_waiter* maybe_wake(
...
TMC_DEF bool try_acquire(std::atomic<size_t>& Value) noexcept;

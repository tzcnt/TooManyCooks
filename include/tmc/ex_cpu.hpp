...
private:
  ...
  TMC_DEF void notify_hint(size_t Priority, size_t ThreadHint);

  TMC_DEF void init_thread_locals(size_t Slot);
  TMC_DEF task_queue_t::ExplicitProducer**
  init_queue_iteration_order(std::vector<std::vector<size_t>> const& Forward);
  TMC_DEF void clear_thread_locals();

  // Returns a lambda closure that is executed on a worker thread
  TMC_DEF auto make_worker(
  ...
  TMC_DEF tmc::detail::InitParams* set_init_params();
...
public:
  ...
  /// different CPU kinds.
  TMC_DEF ex_cpu& set_thread_occupancy(
  ...
  TMC_DEF ex_cpu& fill_thread_occupancy();
  ...
  TMC_DEF ex_cpu& set_thread_pinning_level(tmc::topology::thread_pinning_level Level);
  ...
  /// thread owned by this executor, and passed information about this thread.
  /// This overload requires `TMC_USE_HWLOC`.
  TMC_DEF ex_cpu& set_thread_teardown_hook(
  ...
  TMC_DEF ex_cpu& set_thread_init_hook(std::function<void(size_t)> Hook);

  /// Builder func to set a hook that will be invoked before destruction of each
  /// thread owned by this executor, and passed the ordinal index
  /// [0..thread_count()-1] of the thread.
  TMC_DEF ex_cpu& set_thread_teardown_hook(std::function<void(size_t)> Hook);
...
template <> struct executor_traits<tmc::ex_cpu> {
  ...
  static TMC_DEF tmc::ex_any* type_erased(tmc::ex_cpu& ex);

  static TMC_DEF std::coroutine_handle<>
...
namespace detail {
TMC_DEF tmc::task<void> client_main_awaiter(
...
TMC_DEF int async_main(tmc::task<int>&& ClientMainTask);
} // namespace tmc

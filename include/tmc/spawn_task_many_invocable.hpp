...
namespace tmc {
template <typename result_t, size_t count> struct aw_task_many_invocable;
template <IsNotVoid result_t, size_t count>
struct aw_task_many_invocable<result_t, count> {
  ...
  aw_task_many_invocable(Callable *callable_in,
                         detail::type_erased_executor *executor, size_t prio)
  ...
  ~aw_task_many_invocable() noexcept {
    // If you spawn a function that returns a non-void type,
    // then you must co_await the return of spawn!
    assert(did_await);
  }
  aw_task_many_invocable(const aw_task_many_invocable &) = delete;
  aw_task_many_invocable &operator=(const aw_task_many_invocable &) = delete;
  aw_task_many_invocable(aw_task_many_invocable &&other) = delete;
  // aw_task_many_invocable(aw_task_many_invocable&& other) {
  ...
  // }
  aw_task_many_invocable &operator=(aw_task_many_invocable &&other) = delete;
  // aw_task_many_invocable& operator=(aw_task_many_invocable&& other) {
  //   executor = std::move(other.executor);
  //   continuation = std::move(other.continuation);
  //   wrapped = std::move(other.wrapped);
  //   result = std::move(other.result);
  ...
  inline aw_task_many_invocable &resume_on(detail::type_erased_executor *e) {
  ...
  aw_task_many_invocable &resume_on(Exec &executor) {
  ...
  inline aw_task_many_invocable &run_on(detail::type_erased_executor *e) {
    executor = e;
    return *this;
  }
  template <detail::TypeErasableExecutor Exec>
  aw_task_many_invocable &run_on(Exec &executor) {
  ...
  inline aw_task_many_invocable &with_priority(size_t priority) {
    prio = priority;
    return *this;
  }
};
...
aw_task_many_invocable<result_t, count> spawn_many(Callable *c)
  requires(!std::is_convertible_v<Callable, std::coroutine_handle<>> &&
           std::is_invocable_r_v<result_t, Callable>)
{
  static_assert(count != 0);
  return aw_task_many_invocable<result_t, count>(
      c, detail::this_thread::executor, detail::this_thread::this_task.prio);
}

// For use when count is a runtime parameter
template <typename result_t, typename Callable>
aw_task_many_invocable<result_t, 0> spawn_many(Callable *c, size_t count)
  requires(!std::is_convertible_v<Callable, std::coroutine_handle<>> &&
           std::is_invocable_r_v<result_t, Callable>)
{
  return aw_task_many_invocable<result_t, 0>(
      c, detail::this_thread::executor, detail::this_thread::this_task.prio,
      count);
}

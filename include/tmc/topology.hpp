...
struct cpu_topology {
  ...
  TMC_DECL_HWLOC size_t group_count() const;

  /// The total number of NUMA nodes.
  TMC_DECL_HWLOC size_t numa_count() const;
};
...
TMC_DECL_HWLOC cpu_topology query();
...
public:
  ...
  TMC_DECL_HWLOC void set_core_indexes(std::vector<size_t> Indexes);

  /// Set the allowed group indexes.
  TMC_DECL_HWLOC void set_group_indexes(std::vector<size_t> Indexes);

  /// Set the allowed NUMA indexes.
  TMC_DECL_HWLOC void set_numa_indexes(std::vector<size_t> Indexes);
  ...
  TMC_DECL_HWLOC void set_cpu_kinds(tmc::topology::cpu_kind::value CpuKinds);
  ...
  TMC_DECL_HWLOC topology_filter operator|(topology_filter const& rhs) const;

  /// Gets the allowed core indexes.
  TMC_DECL_HWLOC std::vector<size_t> const& core_indexes() const;

  /// Gets the allowed group indexes.
  TMC_DECL_HWLOC std::vector<size_t> const& group_indexes() const;

  /// Gets the allowed NUMA indexes.
  TMC_DECL_HWLOC std::vector<size_t> const& numa_indexes() const;

  /// Gets the allowed CPU kinds. This is a bitmap that may combine multiple
  /// cpu_kind values.
  TMC_DECL_HWLOC size_t cpu_kinds() const;
};
...
TMC_DECL_HWLOC void pin_thread(topology_filter const& Allowed);

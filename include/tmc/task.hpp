...
template <typename result_t> struct mt1_continuation_resumer {
  ...
  constexpr std::coroutine_handle<> await_suspend(
      std::coroutine_handle<task_promise<result_t>> handle) const noexcept {
...
template <IsNotVoid result_t> struct task_promise<result_t> {
  // coroutine impls
  task_promise()
      : continuation{nullptr}, done_count{nullptr}, result_ptr{nullptr},
        continuation_executor{this_thread::executor} {}
  ...
  void return_value(result_t &&value) {
    *result_ptr = std::forward<result_t>(value);
  }
  void return_value(const result_t &value) {
    *result_ptr = std::forward<result_t>(value);
  }
...
template <IsVoid result_t> struct task_promise<result_t> {
  // coroutine impls
  task_promise()
      : continuation{nullptr}, done_count{nullptr},
        continuation_executor{this_thread::executor} {}
  constexpr std::suspend_always initial_suspend() const noexcept { return {}; }
  constexpr mt1_continuation_resumer<result_t> final_suspend() const noexcept {
    return {};
...
void post(E &ex, T &&item, size_t priority)
  requires(
      ...
      std::is_convertible_v<std::invoke_result_t<T>, std::coroutine_handle<>>)
{
...
{
  ex.post_variant(std::coroutine_handle<>([](T t) -> task<void> {
                    t();
                    co_return;
                  }(std::forward<T>(item))),
                  priority);

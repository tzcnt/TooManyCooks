...
aw_task_many_each<Result, Count, TaskIter, size_t>
spawn_many_each(TaskIter&& Begin)
  requires(detail::is_task_result_v<Task, Result>)
{
  static_assert(Count != 0);
  return aw_task_many_each<Result, Count, TaskIter, size_t>(
...
template <
  ...
  typename Result = Task::result_type>
aw_task_many_each<Result, 0, TaskIter, size_t>
spawn_many_each(TaskIter&& Begin, size_t TaskCount)
  requires(detail::is_task_result_v<Task, Result>)
{
  return aw_task_many_each<Result, 0, TaskIter, size_t>(
...
template <
  ...
  typename Result = Task::result_type>
aw_task_many_each<Result, MaxCount, TaskIter, TaskIter>
spawn_many_each(TaskIter&& Begin, TaskIter&& End)
  requires(detail::is_task_result_v<Task, Result>)
{
  return aw_task_many_each<Result, MaxCount, TaskIter, TaskIter>(
...
template <
  typename TaskIter, typename Task = std::iter_value_t<TaskIter>,
  typename Result = Task::result_type>
aw_task_many_each<Result, 0, TaskIter, TaskIter>
spawn_many_each(TaskIter&& Begin, TaskIter&& End, size_t MaxCount)
  requires(detail::is_task_result_v<Task, Result>)
{
  return aw_task_many_each<Result, 0, TaskIter, TaskIter>(
...
aw_task_many_each<Result, Count, FuncIter, size_t>
spawn_many_each(FuncIter&& FunctorIterator)
  requires(detail::is_func_result_v<Functor, Result>)
{
  static_assert(Count != 0);
  return aw_task_many_each<Result, Count, FuncIter, size_t>(
...
template <
  ...
  typename Result = std::invoke_result_t<Functor>>
aw_task_many_each<Result, 0, FuncIter, size_t>
spawn_many_each(FuncIter&& FunctorIterator, size_t FunctorCount)
  requires(detail::is_func_result_v<Functor, Result>)
{
  return aw_task_many_each<Result, 0, FuncIter, size_t>(
...
template <
  ...
  typename Functor = std::iter_value_t<FuncIter>,
  typename Result = std::invoke_result_t<Functor>>
aw_task_many_each<Result, MaxCount, FuncIter, FuncIter>
spawn_many_each(FuncIter&& Begin, FuncIter&& End)
  requires(detail::is_func_result_v<Functor, Result>)
{
  return aw_task_many_each<Result, MaxCount, FuncIter, FuncIter>(
...
template <
  typename FuncIter, typename Functor = std::iter_value_t<FuncIter>,
  typename Result = std::invoke_result_t<Functor>>
aw_task_many_each<Result, 0, FuncIter, FuncIter>
spawn_many_each(FuncIter&& Begin, FuncIter&& End, size_t MaxCount)
  requires(detail::is_func_result_v<Functor, Result>)
{
  return aw_task_many_each<Result, 0, FuncIter, FuncIter>(
...
using aw_task_many_each_run_early =
...
template <typename Result, size_t Count, typename IterBegin, typename IterEnd>
class [[nodiscard("You must use the aw_task_many_each<Result> by one of: 1. "
                  "co_await 2. run_early()")]] aw_task_many_each
    : public detail::run_on_mixin<
        aw_task_many_each<Result, Count, IterBegin, IterEnd>>,
      public detail::resume_on_mixin<
        aw_task_many_each<Result, Count, IterBegin, IterEnd>>,
      public detail::with_priority_mixin<
        aw_task_many_each<Result, Count, IterBegin, IterEnd>> {
  friend class detail::run_on_mixin<aw_task_many_each>;
  friend class detail::resume_on_mixin<aw_task_many_each>;
  friend class detail::with_priority_mixin<aw_task_many_each>;
...
public:
  ...
  /// It is recommended to call `spawn_many_each()` instead of using this
  /// constructor directly.
  aw_task_many_each(IterBegin TaskIterator, IterEnd Sentinel, size_t MaxCount)
  ...
  // aw_task_many_each(IterBegin TaskIterator)
  //   requires(Count != 0)
  //     : aw_task_many_each(TaskIterator, Count) {}

  aw_task_many_each_impl<Result, Count> operator co_await() && {
#ifndef NDEBUG
    assert(!did_await);
    did_await = true;
#endif
    ...
    if constexpr (std::is_convertible_v<IterEnd, size_t>) {
      ...
      return aw_task_many_each_impl<Result, Count>(
...
#ifndef NDEBUG
  ~aw_task_many_each() noexcept { assert(did_await); }
#endif
  aw_task_many_each(const aw_task_many_each&) = delete;
  aw_task_many_each& operator=(const aw_task_many_each&) = delete;
  aw_task_many_each(aw_task_many_each&& Other) = delete;
  ...
  aw_task_many_each& operator=(aw_task_many_each&& Other) = delete;
  //    {
  //     iter = std::move(Other.iter);
  //     sentinelOrCount = std::move(Other.sentinelOrCount);
  //     executor = std::move(Other.executor);
  ...
  inline aw_task_many_each_run_early<Result, Count> run_early() && {
#ifndef NDEBUG
    assert(!did_await);
    did_await = true;
#endif
    return aw_task_many_each_run_early<Result, Count>(
